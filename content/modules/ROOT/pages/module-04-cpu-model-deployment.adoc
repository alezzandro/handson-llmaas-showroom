...
[.bordershadow]
image::04/01-create_project_model_serving.png[width="100%"]
...
[.bordershadow]
image::04/01-single_model_serving.png[width="100%"]
...
[.bordershadow]
image::04/01-model_route.png[width="100%"]
...
[source,text]
----
https://qwen-cpu-server-{user}-model-serving.{openshift_cluster_ingress_domain}/v1
----
...
model = ChatOpenAI(
    model_name="qwen-cpu-server",
    openai_api_base="https://qwen-cpu-server.{user}-model-serving.svc.cluster.local/v1",
...
* `model_name`: Must match the model name from your deployment (`qwen-cpu-server`)
...
* **Module 2**: GPU-accelerated models for high-performance inference
