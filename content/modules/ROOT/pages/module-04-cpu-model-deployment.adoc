...
[.bordershadow]
image::07/01-rhoai-dashboard-home.png[OpenShift AI Dashboard Home,width=75%]

== Step 2: Navigate to Model Serving

OpenShift AI provides centralized model serving capabilities through the dashboard.

* In the left navigation menu, click on **Model Serving**

* This opens the model serving overview page where you can see all deployed models across your projects

[.bordershadow]
image::07/02-model-serving-overview.png[Model Serving Overview,width=75%]

== Step 3: Select Your Project

You'll deploy this model in your existing project namespace.

* Look for the project selector dropdown (typically at the top of the page)

* Select your project: `{user}-ai-models`
+
NOTE: This is the same project where you deployed the Granite model in Module 1. You can have multiple models deployed in the same project.

* Once selected, you should see any existing model servers in this project

[.bordershadow]
image::07/03-project-selection.png[Select Project,width=75%]

== Step 4: Create a New Model Server
...
* In the **Model server name** field, enter: `qwen-cpu-server`

* For **Serving runtime**, select the custom CPU-optimized vLLM runtime from the dropdown
+
Look for: **vLLM CPU Runtime** or similar (the exact name depends on how your administrator configured it)
+
[.bordershadow]
image::07/04-select-runtime.png[Select vLLM CPU Runtime,width=75%]
...
* Expand the **Resources** section if it's collapsed
...
[.bordershadow]
image::07/05-configure-resources.png[Configure Resources,width=75%]
...
* Scroll down to the **Deploy model** section

* In the **Model name** field, enter: `qwen-0.5b-instruct`
+
This is the name that will appear in API requests

* For **Model framework**, select **vLLM** (should be pre-selected based on your runtime choice)

* In the **Model location** section, you need to specify where the model is stored

* Choose **Model location type**: **Object storage** or **Model registry** depending on your environment
+
For this exercise, we'll use a pre-configured model from a container image

* Select **Existing data connection** or **New data connection** as appropriate

* In the **Path** or **Model path** field, enter the model identifier:
+
[source,text]
----
...
[.bordershadow]
image::07/06-specify-model-location.png[Specify Model Location,width=75%]

== Step 7: Configure Model Parameters

vLLM supports various configuration parameters for optimizing inference.

* Expand the **Model parameters** or **Advanced settings** section

* Configure the following parameters for CPU optimization:
+
[source,yaml]
----
--max-model-len: 2048
...
[.bordershadow]
image::07/07-model-parameters.png[Model Parameters,width=75%]
...
[.bordershadow]
image::07/08-deployment-initiated.png[Deployment Initiated,width=75%]
...
* You should see the model server appear in the model serving list with a status indicator
...
[.bordershadow]
image::07/09-deployment-progress.png[Deployment Progress,width=75%]

* You can click on the model server name to see more details

* Check the **Events** or **Logs** tab to monitor the startup process
...
[.bordershadow]
image::07/10-deployment-ready.png[Deployment Ready,width=75%]
...
* You should see your existing project `{user}-ai-models` (the same namespace where your model is deployed)

* Click on the project name to open it

[.bordershadow]
image::07/11a-open-project.png[Open Data Science Project,width=75%]

* In the project view, locate the **Workbenches** section and click **Create workbench**

[.bordershadow]
image::07/11b-create-workbench.png[Create Workbench,width=75%]

* Configure the workbench with the following settings:
** **Name**: `qwen-test-notebook`
** **Notebook image**: Select **Standard Data Science** (this includes Python, Jupyter, and common ML libraries)
...
[.bordershadow]
image::07/11c-workbench-config.png[Workbench Configuration,width=75%]
...
[.bordershadow]
image::07/11d-workbench-starting.png[Workbench Starting,width=75%]

* Once running, click **Open** to launch the Jupyter interface

[.bordershadow]
image::07/11e-workbench-running.png[Workbench Running,width=75%]
...
[.bordershadow]
image::07/11f-jupyter-git.png[Jupyter Git Interface,width=75%]
...
[.bordershadow]
image::07/11g-git-clone.png[Git Clone Dialog,width=75%]
...
[source,text]
----
handson-llmaas-showroom/exercises/07-cpu-model-test/
----

* You should see a Python notebook file: `test_qwen_langchain.ipynb`

[.bordershadow]
image::07/11h-exercise-folder.png[Exercise 07 Folder,width=75%]

* Double-click the notebook to open it

[.bordershadow]
image::07/11i-notebook-opened.png[Notebook Opened,width=75%]
...
[.bordershadow]
image::07/11j-configure-endpoint.png[Configure Endpoint URL,width=75%]
...
[.bordershadow]
image::07/11k-run-cells.png[Run All Cells,width=75%]
...
[.bordershadow]
image::07/11l-notebook-output.png[Notebook Output,width=75%]
...
* Switch to **Developer** perspective (top-left dropdown)

* Select your project: `{user}-ai-models`

* Click on **Topology** to see a visual representation

* You should see a new deployment for `qwen-cpu-server`

[.bordershadow]
image::07/12-topology-view.png[Topology View,width=75%]

* Click on the deployment to open the details panel:
** **Pods**: Shows 1 pod running with CPU resources
** **Resources**: No GPU allocation (CPU-only)
...
[.bordershadow]
image::07/13-pod-logs.png[Pod Logs,width=75%]

* Navigate to **Observe** → **Metrics** in the OpenShift Console

* Select your project and pod to view resource usage:
** CPU utilization during inference
** Memory consumption
** Network traffic

[.bordershadow]
image::07/14-resource-metrics.png[Resource Metrics,width=75%]
