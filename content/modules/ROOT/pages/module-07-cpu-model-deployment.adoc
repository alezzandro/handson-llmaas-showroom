...
[.bordershadow]
image::new/vllmcpu-ocpai.png[OpenShift AI Dashboard Home,width=75%]

== Step 2: Navigate to Data Science Projects

OpenShift AI provides centralized model serving capabilities through the dashboard.

* In the left navigation menu, click on **Data Science Project**

* Then create a new Data Science Project clicking the respective button, assign a name to your project such as: `{user}-vllmcpu`

[.bordershadow]
image::new/vllmcpu-ocpai2.png[Model Serving Overview,width=75%]

== Step 3: Move to the Models subsection

You'll deploy this model in the just created project namespace.

[.bordershadow]
image::new/vllmcpu-ocpai3.png[Select Project,width=75%]
...
* Click the **Select single-model** button 

* Then click the **Deploy model** button

* You'll see a form to configure your model server

* In the **Model deployment name** field, enter: `qwen-cpu`

* For **Serving runtime**, select the custom CPU-optimized vLLM runtime from the dropdown
+
Look for: **vLLM CPU ServingRuntime** or similar

* Select **Medium** as Model server size.

* Leave **Model route** disabled (not checked) to not expose our model outside of the OpenShift cluster

* Set the **Number of model replicas** to `1`

* Select **OCI compliant registry** as default Connection type.

* Set **quayio** as name for the connection

* Set `{}` as value in the **Secret details**

* Set **quay.io** as Registry host

* Then place this URL `quay.io/redhat-ai-services/modelcar-catalog:qwen2.5-0.5b-instruct` in the Model URI input field.
+ We are going to use a pre-built OCI image containing our base Qwen model.
...
TIP: You could even take a look to the underlying OpenShift cluster to see which kind of resources the OpenShift AI is scheduling on the OpenShift cluster.
...
[source,text]
----
https://qwen-cpu.{user}-vllmcpu.svc.cluster.local
----

[.bordershadow]
image::new/vllmcpu-model-deployed.png[Workbench Configuration,width=75%]

* This is your OpenAI-compatible endpoint for the Qwen model

== Step 8: Test the Model with a Jupyter Notebook
...
* You should see your existing project `{user}-vllmcpu` (the same namespace where your model is deployed)
...
** **Notebook image**: Select **Minimal Data Science** (this includes Python, Jupyter, and common ML libraries)
...
[.bordershadow]
image::new/vllmcpu-wbstarting.png[Workbench Starting,width=75%]

* Once running, click on its name to launch the Jupyter interface
+ If requested input your OpenShift credentials

[.bordershadow]
image::new/vllmcpu-jupyter.png[Workbench Running,width=75%]
...
[.bordershadow]
image::new/vllmcpu-jupyternotebook.png[Notebook Opened,width=75%]
...
* `model_name`: Must match the model name from your deployment (`qwen-cpu
`)
...
----
+
Replace `{user}` with your actual username (e.g., `student1`, `student2`, etc.)
...
* Select your project: `{user}-vllmcpu`

* Click on **Workloads -> Topology** to see a visual representation

* You should see a new deployment for `qwen-cpu`

[.bordershadow]
image::new/vllmcpu-ocp-topology.png[Topology View,width=75%]
